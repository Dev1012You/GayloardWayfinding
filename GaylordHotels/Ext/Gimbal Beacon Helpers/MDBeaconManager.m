...
@property (nonatomic, strong) MDCustomTransmitter *lastActiveBeacon;
@end
...
- (void)addBeacon:(GMBLBeacon *)beacon shouldUpdate:(BOOL)shouldUpdate
{
    ...
    if (!customBeacon)
    {
        ...
        [customBeacon fillValuesFrom:beacon];
...
{
    NSArray *beacons = [NSArray arrayWithArray:self.allBeacons];
    
    NSIndexSet *foundBeacons = [beacons indexesOfObjectsPassingTest:^BOOL(id obj, NSUInteger idx, BOOL *stop) {
    ...
    if (foundBeacons.count == 1 && beacons.count > [foundBeacons firstIndex])
    {
        return [beacons objectAtIndex:[foundBeacons firstIndex]];
    }
    else
    {
        return nil;
...
- (NSArray*)compareBeacons:(NSArray*)beaconCollection
{
    ...
    NSNumber *defaultRSSI = [[self.userDefaults objectForKey:MTP_BeaconOptions] objectForKey:MTP_DefaultNilRSSI];
    NSNumber *relevantBeaconCount = [[self.userDefaults objectForKey:MTP_BeaconOptions] objectForKey:MTP_RelevantBeaconCount] ;
    ...
    [sortedArray enumerateObjectsUsingBlock:^(id obj, NSUInteger idx, BOOL *stop)
    {
        if ([obj RSSI].integerValue < 0 &&
            [obj RSSI].integerValue > [defaultRSSI integerValue] &&
            relevantBeacons.count < MIN(sortedArray.count,relevantBeaconCount.integerValue))
        {
            [relevantBeacons addObject:obj];
        }
        
        // if we havent seen the beacon in more than twenty seconds, reset its relevant values
        if ([[obj lastSeen] timeIntervalSinceNow] < -20)
        {
            [obj setLastSeen:nil];
            [obj setRSSI:defaultRSSI];
            [obj setReadingHistory:[NSArray new]];
        }
    }];
...
- (NSArray*)compareBeacons:(NSArray*)beaconCollection
{
    NSArray *sortedArray = [beaconCollection sortedArrayUsingComparator:^NSComparisonResult(id  _Nonnull obj1, id  _Nonnull obj2) {
...
- (void)compareBeacons
{
    [self identifyActiveBeacon];
}

- (MDCustomTransmitter *)identifyActiveBeacon
{
    NSArray *relevantBeacons = [self compareBeacons:[NSArray arrayWithArray:self.allBeacons]];
    
    self.nearbyBeacons = [NSMutableArray arrayWithArray:relevantBeacons];
    self.activeBeacon = [self.nearbyBeacons firstObject];

    if (self.activeBeacon != nil && ![self.lastActiveBeacon.identifier isEqualToString:self.activeBeacon.identifier])
    {
        // active beacon changed
        self.lastActiveBeacon = self.activeBeacon;
    }
...
-(void)transmitBeaconData:(NSArray*)beaconCollection updateType:(NSString*)updateType {
    ...
    return;
#warning NOT CURRENTLY USING ANY OTHER BEACON FUNCTIONS
    /*
...
{
    [self getEventBeacons:nil];
}

- (void)getEventBeacons:(void(^)(NSArray *fetchedBeacons))completionHandler
{
    __weak __typeof(&*self)weakSelf = self;
                                    ...
                                    completionHandler:^(NSData *data, NSURLResponse *response, NSError *error)
    {
        ...
        if ([responseObject isKindOfClass:[NSDictionary class]])
        {
            NSArray *eventBeacons = [[responseObject objectForKey:@"data"] objectForKey:@"beacons"];
...
//                DLog(@"\nself.allBeacon %@", weakSelf.allBeacons);
                ...
                {
                    completionHandler(self.allBeacons);
                }
                
                NSMutableArray *beaconsCopy = [self.allBeacons copy];
                ...
                if (cachesDirectory)
                {
                    NSURL *beaconCacheURL = [cachesDirectory URLByAppendingPathComponent:@"GAHBeacons" isDirectory:true];
                    NSError *beaconCacheCreationError = nil;
                    if ([[NSFileManager defaultManager] createDirectoryAtURL:beaconCacheURL
                                                 withIntermediateDirectories:true
                                                                  attributes:nil
                                                                       error:&beaconCacheCreationError])
                    {
                        if (![NSKeyedArchiver archiveRootObject:beaconsCopy
                                                         toFile:[beaconCacheURL.path stringByAppendingString:@"beacons.archive"]])
                        {
                            DLog(@"\nwrite failed");
                        }
                    }
                    ...
                    {
                        DLog(@"\nbeaconCacheCreationError %@", beaconCacheCreationError);
                    }
                }
            }
            else
...
- (NSArray *)localBeaconCollection
{
    NSURL *cachesDirectory = [[[NSFileManager defaultManager] URLsForDirectory:NSCachesDirectory inDomains:NSUserDomainMask] firstObject];
    NSURL *beaconCacheURL = [cachesDirectory URLByAppendingPathComponent:@"GAHBeacons" isDirectory:true];
    NSString *beaconsCollection = [beaconCacheURL.path stringByAppendingString:@"beacons.archive"];
    NSArray *unarchived = [NSKeyedUnarchiver unarchiveObjectWithFile:beaconsCollection];
    
    if (unarchived.count > 0)
    {
        NSLog(@"\nunarchived beacons %@",unarchived);
    }
