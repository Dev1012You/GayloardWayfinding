...
- (void)beaconManager:(GMBLBeaconManager *)manager didReceiveBeaconSighting:(GMBLBeaconSighting *)sighting
{
    ...
    if (RSSI.integerValue < 0) {
        ...
        beacon.lastSeen = [NSDate date];
        
//        NSLog(@"\nrssi %@ -- beacon %@ and rssi %@",RSSI,beacon.identifier,beacon.RSSI);
...
- (void)registerForNotifications
{
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(didReceiveLoginNotification:)
                                                 name:@"MTP_StartRangingBeacons"
                                               object:nil];
    
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(didReceiveLogoutNotification:)
                                                 name:@"MTP_StopRangingBeacons"
                                               object:nil];
}
...
- (void)updateAPI {
    ...
    /*
    NSArray *allBeacons = [NSArray arrayWithArray:self.beaconManager.allBeacons];
    __block NSMutableArray *oldBeacons = [NSMutableArray new];
    NSDate *twoHoursOld = [NSDate dateWithTimeIntervalSinceNow:-60*60*2];
    [allBeacons enumerateObjectsUsingBlock:^(id obj, NSUInteger idx, BOOL *stop) {
        NSDate *beaconLastSeen = [obj lastSeen];
        if ([[beaconLastSeen earlierDate:twoHoursOld]isEqualToDate:beaconLastSeen])
        {
            [oldBeacons addObject:beaconLastSeen];
        }
    }];
    [self.beaconManager transmitBeaconData:oldBeacons updateType:kBeaconDeleteUsers];
     */
}
