...
@implementation GAHFuzzySearch

+ (CGFloat) scoreString:(NSString *)originalString against:(NSString *)otherString
{
    return [self scoreString:originalString against:otherString fuzziness:nil];
}

+ (CGFloat) scoreString:(NSString *)originalString against:(NSString *)otherString fuzziness:(NSNumber *)fuzziness
{
    return [self scoreString:originalString against:otherString fuzziness:fuzziness options:NSStringScoreOptionNone];
}
...
+ (CGFloat) scoreString:(NSString *)originalString against:(NSString *)anotherString fuzziness:(NSNumber *)fuzziness options:(NSStringScoreOption)options
{
    ...
    NSString *string = [[[originalString decomposedStringWithCanonicalMapping] componentsSeparatedByCharactersInSet:invalidCharacterSet] componentsJoinedByString:@""];
    ...
    for(uint index = 0; index < otherStringLength; index++){
        ...
        if(rangeChrLowercase.location == NSNotFound && rangeChrUppercase.location == NSNotFound){
            if(fuzziness){
                ...
                return totalCharacterScore / otherStringLength; // this is an error!
            }
